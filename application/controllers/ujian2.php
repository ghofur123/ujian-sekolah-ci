	...
	public function load(){
		if(empty($_GET["act"])){
			...
			if($_GET["act"] == "insert"){
				...
				$queryDataRead = array(
					"tampilData" => $this->crud_function_model->readData("ujian", "", "id_ujian like '%$_GET[id]%'  || nama_ujian like '%$_GET[id]%' || kelas_id like '%$_GET[id]%' || token like '%$_GET[id]%' || guru_id like '%$_GET[id]%' || materi_id like '%$_GET[id]%' || jumlah_ujian like '%$_GET[id]%' || persen_multiple_coice like '%$_GET[id]%' || persen_essay like '%$_GET[id]%' ", "id_ujian desc"),
					"idDb" => "id_ujian"
				);
				$this->load->view("admin/content/ujian/table", $queryDataRead);
			}// end search
